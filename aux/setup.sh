...
if [ "$answerFrontEnd" == "y" ]; then
  ...
  git clone https://github.com/SatiricalChan/PenumbraSatirical.git ../src/fe
  cd ../src/fe
  git checkout master
  cd ../../aux
...
if [ "$answerStable" == "y" ]; then
  ...
  echo "Changed to latest stable version: 2.1.x"
...
if [ "$answerLocation" == "y" ]; then
  ...
  git clone https://github.com/SatiricalChan/SatiricalChan-LocationDownloader.git ../src/be/locationData

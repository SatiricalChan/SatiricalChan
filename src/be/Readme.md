...
# Application usage
`boot.js` is the main file, run it using Node.js to start the system. Keep in mind that if you ran `aux/setup.sh`, you can just run the `satiricalchan` command or start the `satiricalchan` service.
...
Examples:
`satiricalchan -r -nd` = rebuild all pages and do not start a server.
`satiricalchan -ca -l login -p pass -gr 0` = create an account with login login, password pass with root global role and then start the server.
`satiricalchan -rf -rb -nd` = reload the saved front-page and the default banner and do not start a server.
